...
export default function DoctorLeavesPage() {
  ...
  const loadLeaves = useCallback(async () => {
    setLoading(true);
    try {
      const res = await schedulingApi.listLeaves({ doctor_id: doctorId, page: 1, page_size: 50 });
      setLeaves(res.data);
    } catch (err: unknown) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.warning && res.warning.appointments_affected > 0) {
        setSuccess(`Leave created. Warning: ${res.warning.appointments_affected} appointment(s) may need rescheduling.`);
      }
      setShowModal(false);
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Doctor Leave Management</h2>
            <p className="mt-1 text-sm text-gray-600">
              Manage temporary leave records. Leave overrides the recurring weekly schedule.
            </p>
          </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="font-medium text-gray-900">{leave.leave_type.replace(/_/g, " ")}</span>
